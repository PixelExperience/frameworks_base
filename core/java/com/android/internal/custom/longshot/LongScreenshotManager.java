...
public final class LongScreenshotManager {
    ...
    public void stopLongshot(int reason) {
        try {
            if (mService != null) {
                mService.stopLongshot(reason);
            }
        } catch (RemoteException e) {
    ...
    public void notifyLongshotScrollChanged(int x, int y, int oldx, int oldy) {
        try {
            if (mService != null) {
                mService.notifyLongshotScrollChanged(x, y, oldx, oldy);
            }
        } catch (RemoteException e) {
            Log.w(TAG, "Remote exception in notifyLongshotScrollChanged: ", e);
        }
    }
    ...
    public void notifyScrollViewSearchComplete(int top, int bottom, int scrollViewType) {
        try {
            if (mService != null) {
                mService.notifyScrollViewSearchComplete(top, bottom, scrollViewType);
            }
        } catch (RemoteException e) {
            Log.w(TAG, "Remote exception in notifyScrollViewSearchComplete: ", e);
        }
    }
    ...
    public void notifyWindowLayerChange(IBinder windowToken) {
        try {
            if (mService != null) {
                mService.notifyWindowLayerChange(windowToken);
            }
        } catch (RemoteException e) {
            Log.w(TAG, "Remote exception in notifyWindowLayerChange: ", e);
        }
    }
