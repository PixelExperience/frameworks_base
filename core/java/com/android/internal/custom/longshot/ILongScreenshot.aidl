...
interface ILongScreenshot {
    ...
    void notifyLongshotScrollChanged(int x, int y, int oldx, int oldy);

    void notifyScroll(boolean isOverScroll);

    void notifyScrollViewSearchComplete(int top, int bottom, int scrollViewType);

    void notifyScrollViewTop(int viewTop);

    void notifyWindowLayerChange(IBinder windowToken);

    void onUnscrollableView();

    void start(in ILongScreenshotCallback callback);

    void stopLongshot(int reason);
}