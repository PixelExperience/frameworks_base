...
public class NetworkTraffic extends TextView {
    ...
    private Handler mTrafficHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Log.d(TAG, "getMyMode() = " + getMyMode());
            Log.d(TAG, "mMode = " + mMode);
            Log.d(TAG, "mScreenOn = " + (mScreenOn ? "true" : "false"));

            final boolean enabled = mMode == getMyMode() && mScreenOn && isConnectionAvailable();
            final boolean shouldHide = !enabled ||
                    (mAutoHide &&
                        mRxKbps < mAutoHideThreshold && mTxKbps < mAutoHideThreshold);

            Log.d(TAG, "shouldHide = " + (shouldHide ? "true" : "false"));
            Log.d(TAG, "enabled = " + (enabled ? "true" : "false"));
            if (shouldHide) {
                setText("");
    ...
    protected void updateTrafficDrawable() {
        ...
        if (mMode == getMyMode() && (mIndicatorUp || mIndicatorDown)) {
            if (mIndicatorUp) {
