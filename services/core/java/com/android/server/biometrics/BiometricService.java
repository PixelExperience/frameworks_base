...
public class BiometricService extends SystemService {
    ...
    private void handleOnUseFacePressed() {
        ...
        currentAuthSession.mModality = TYPE_FACE;
        mCurrentAuthSession = null;
        cancelInternal(null /* token */, null /* package */, Binder.getCallingUid(),
                Binder.getCallingPid(), UserHandle.getCallingUserId(),
    ...
    private void switchToFaceModality(AuthSession authSession) {
        ...
        boolean requireConfirmation = mSettingObserver.getFaceAlwaysRequireConfirmation(authSession.mUserId);
        ...
        mPendingAuthSession.mRequireConfirmation = requireConfirmation;
    ...
    private void authenticateInternal(IBinder token, long sessionId, int userId,
        ...
        if ((modality & TYPE_FACE) != 0) {
            ...
            requireConfirmation = mSettingObserver.getFaceAlwaysRequireConfirmation(userId);
        }
        // Generate random cookies to pass to the services that should prepare to start
        // authenticating. Store the cookie here and wait for all services to "ack"
        // with the cookie. Once all cookies are received, we can show the prompt
