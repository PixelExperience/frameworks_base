...
public class CustomFaceService {
    ...
    final SparseArray<IMotoFaceService> mServices = new SparseArray<>();
    final IMotoFaceServiceReceiver mReceiver = new IMotoFaceServiceReceiver.Stub() {
        @Override
        public void onEnrollResult(int faceId, int userId, int remaining) {
            mHandler.post(() -> {
                mFaceService.handleEnrollResult(new Face(
                        ...
                        mContext, userId), faceId, DEVICE_ID), remaining);
            });
        }
        ...
        public void onAuthenticated(int faceId, int userId, byte[] token) {
            mHandler.post(() -> {
                ...
                if (token == null || faceId <= 0) {
                    ...
                    mFaceService.handleAuthenticated(authenticated, new Face("", 0, DEVICE_ID), null);
                    return;
                }
                Face face = new Face("", faceId, DEVICE_ID);
        ...
        public void onAcquired(int userId, int acquiredInfo, int vendorCode) {
            mHandler.post(() -> {
                mFaceService.handleAcquired(DEVICE_ID, acquiredInfo, vendorCode);
            });
        }
        ...
        public void onError(int error, int vendorCode) {
            mHandler.post(() -> {
                mFaceService.handleError(DEVICE_ID, error, vendorCode);
            });
        }
            ...
            mHandler.post(() -> {
                if (faceIds.length > 0) {
                    for (int i = 0; i < faceIds.length; i++) {
                        mFaceService.handleRemoved(new Face("", faceIds[i], DEVICE_ID), (faceIds.length - i) - 1);
                    }
                    return;
                }
                mFaceService.handleRemoved(new Face("", 0, DEVICE_ID), 0);
            });
        }
        ...
        public void onEnumerate(int[] faceIds, int userId) throws RemoteException {
            mHandler.post(() -> {
                if (faceIds.length > 0) {
                    for (int i = 0; i < faceIds.length; i++) {
                        mFaceService.handleEnumerate(new Face("", faceIds[i], DEVICE_ID), (faceIds.length - i) - 1);
                    }
                    return;
                }
    ...
    public int authenticate(long operationId) {
        IMotoFaceService service = getService(mCurrentUserId);
        if (service != null) {
            try {
        ...
        if (service != null) {
            mServiceHandler.post(() -> {
                try {
                    ...
                    mHandler.post(() -> {
                        mFaceService.handleError(DEVICE_ID, 8, 0);
                    });
                }
            });
            return BIOMETRIC_SUCCESS;
    ...
    public IMotoFaceService getService(int userId) {
    ...
    private Intent getServiceIntent() {
        Intent intent = new Intent("com.motorola.faceunlock.BIND");
        intent.setComponent(ComponentName.unflattenFromString(
                "com.motorola.faceunlock/com.motorola.faceunlock.service.FaceAuthService"));
        return intent;
    }

    public String getServicePackageName() {
        return "com.motorola.faceunlock";
    }
    ...
    private class FaceServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            IMotoFaceService faceService = IMotoFaceService.Stub.asInterface(service);
            if (faceService != null) {
