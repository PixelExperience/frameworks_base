...
public class FaceService extends BiometricServiceBase {
    ...
    private boolean mUseMotoFaceUnlockService = false;
    ...
    private boolean isMotoFaceServiceEnabled() {
        ...
        if (!this.mUseMotoFaceUnlockService) {
            return false;
        }
    ...
    private IMotoFaceService getMotoFaceService(int userId) {
        ...
        return this.mMotoFaceServices.get(this.mCurrentUserId);
    }
    ...
    private boolean bindMotoFaceAuthService(int userId) {
        ...
        if (!isMotoFaceServiceEnabled()) {
            ...
            return false;
        } else if (this.mIsMotoServiceBinding) {
            ...
            if (userId != -10000 && getMotoFaceService(userId) == null) {
                try {
                    ...
                    if (result) {
                        this.mIsMotoServiceBinding = true;
                    }
                    return result;
    ...
    private final class FaceServiceWrapper extends IFaceService.Stub {
        ...
        public List<Face> getEnrolledFaces(int userId, String opPackageName) {
            ...
            return .getEnrolledTemplates(userId);
        }
        ...
        public boolean hasEnrolledFaces(int userId, String opPackageName) {
            ...
            return .hasEnrolledBiometrics(userId);
        }
        ...
        public long getAuthenticatorId(String opPackageName) {
            ...
            return .getAuthenticatorId(opPackageName);
        }
        ...
        private int getFirstTemplateForUser(int user) {
            final List<Face> faces = .getEnrolledTemplates(user);
            if (!faces.isEmpty()) {
    ...
    protected boolean hasReachedEnrollmentLimit(int userId) {
        ...
        final int enrolled = .getEnrolledTemplates(userId).size();
        if (enrolled >= limit) {
    ...
    private long startGenerateChallenge(IBinder token) {
        if (mUseMotoFaceUnlockService) {
            ...
            if (service != null) {
                try {
                    return service.generateChallenge(600);
                } catch (RemoteException e) {
                    e.printStackTrace();
                    return 0;
                }
