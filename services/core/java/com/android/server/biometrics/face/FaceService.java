...
public class FaceService extends BiometricServiceBase {
    ...
    private static final int MOTO_DEVICE_ID = 1008;
            ...
            mHandler.post(new Runnable() {
                ...
                public final void run() {
                    ...
                    if (token == null || faceId <= 0) {
                        ...
                        FaceService.super.handleAuthenticated(authenticated, new Face("", 0, MOTO_DEVICE_ID), null);
                        return;
                    }
    ...
    private static final boolean mUseMotoFaceUnlockService = FaceUnlockUtils.isFaceUnlockSupported();
        ...
        if (!isMotoFaceServiceEnabled()) {
            ...
            if (userId != UserHandle.USER_NULL && getMotoFaceService(userId) == null) {
                try {
                    Intent intent = new Intent(BIND_MOTOFACEID_ACTION);
                    intent.setClassName(PACKAGE_MOTOFACEID_PACKAGE_NAME, PACKAGE_MOTOFACEID_SERVICE_NAME);
                    boolean result = getContext().bindServiceAsUser(intent, new MotoFaceServiceConnection(userId), 1, UserHandle.of(userId));
                    if (result) {
    ...
    public void checkPermission(String permission, String packageName) {
        if (!mUseMotoFaceUnlockService || !PACKAGE_MOTOFACEID_PACKAGE_NAME.equals(packageName)) {
    ...
    private final class FaceAuthClient extends AuthenticationClientImpl {
        ...
        public boolean shouldFrameworkHandleLockout() {
            return mUseMotoFaceUnlockService;
        }
    ...
    private final class FaceServiceWrapper extends IFaceService.Stub {
        ...
        public void cancelEnrollment(final IBinder token) {
            checkPermission(MANAGE_BIOMETRIC, PACKAGE_MOTOFACEID_PACKAGE_NAME);
    ...
    protected void updateActiveGroup(int userId, String clientPackage) {
        if (mUseMotoFaceUnlockService) {
            ...
            if (service != null) {
                try {
                    long authId = 0;
                    if (hasEnrolledBiometrics(this.mCurrentUserId)) {
